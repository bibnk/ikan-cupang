...
(function () {
    ...
    function onComplete(d) {
        ...
        // Show resolve unreg button if there are unreg domains
        if (d.unreg > 0 && currentJobId) {
            showResolveUnregBtn(currentJobId, d.unreg);
        }
    }

    function showResolveUnregBtn(jobId, unreg) {
        // Remove existing
        var existing = document.getElementById("resolve-unreg-container");
        if (existing) existing.remove();

        var container = document.createElement("div");
        container.id = "resolve-unreg-container";
        container.style.cssText = "margin-top:12px; padding:12px; background:var(--bg-card); border:1px dashed var(--accent-primary); border-radius:var(--radius-md); text-align:center;";

        container.innerHTML =
            '<button id="resolve-unreg-btn" class="btn btn-accent" style="margin-bottom:8px;">' +
            '🔍 Resolve ' + unreg + ' Unreg Domain ke IMAP Config</button>' +
            '<div id="resolve-unreg-progress" style="display:none; font-size:13px; color:var(--text-muted);"></div>';

        // Insert after download links section
        var downloadSection = document.querySelector(".download-links");
        if (downloadSection) {
            downloadSection.parentNode.insertBefore(container, downloadSection.nextSibling);
        } else {
            document.querySelector(".progress-section").appendChild(container);
        }

        document.getElementById("resolve-unreg-btn").addEventListener("click", function() {
            startResolveUnreg(jobId);
        });
    }

    function startResolveUnreg(jobId) {
        var btn = document.getElementById("resolve-unreg-btn");
        var prog = document.getElementById("resolve-unreg-progress");
        btn.disabled = true;
        btn.textContent = "⏳ Resolving...";
        prog.style.display = "block";
        prog.textContent = "Memulai resolve...";

        fetch(APP_BASE + "/api/resolve-unreg/" + jobId, { method: "POST" })
            ...
            .then(function(data) {
                ...
                prog.textContent = "Resolving " + data.total_domains + " domain...";
                pollResolveStatus(jobId);
            })
            .catch(function(err) {
    ...
    function pollResolveStatus(jobId) {
        var prog = document.getElementById("resolve-unreg-progress");
        var btn = document.getElementById("resolve-unreg-btn");

        var poll = setInterval(function() {
            fetch(APP_BASE + "/api/resolve-unreg-status/" + jobId)
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.status === "running") {
                        var phase = data.phase === "mx" ? "MX Lookup" : "IMAP Test";
                        prog.textContent = phase + ": " + (data.checked || 0) + "/" + (data.phase_total || "?") + " | Ditemukan: " + (data.found || 0);
                    } else if (data.status === "done") {
                        clearInterval(poll);
                        var msg = "✅ Selesai! Ditambahkan " + data.added + " domain ke IMAP config";
                        msg += " (MX: " + (data.mx_mapped || 0) + ", Direct: " + (data.direct_found || 0) + ")";
                        msg += " | Total config: " + data.total_config;
                        prog.textContent = msg;
                        prog.style.color = "var(--color-live)";
                        btn.textContent = "✅ Done (" + data.added + " ditambahkan)";
                        btn.disabled = true;
